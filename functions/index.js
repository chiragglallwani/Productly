...
app.get('/read', (req, res) => {
  (async() => {
    try{
      const response = await db.collection('Products').get().then((querySnapShot) => {
...
app.get('/readOnlyCategory', (req,res) => {
  (async() => {
    try{
      let category = [];
      await db.collection('Products').orderBy('category').get().then(data => {
        data.docs.map(doc => {
...
app.get('/filter/:category', (req, res) => {
  (async() => {
    try{
      const response = await db.collection('Products').where('category', '==', req.params.category).get().then((queryResult) => {
